...
if (!process.env.VITE_DEV_SERVER_URL) try {
	autoUpdater = require_main().autoUpdater;
...
function createWindow() {
	...
	if (autoUpdater && isWindowSafe) {
		...
		autoUpdater.on("update-available", (info) => {
			if (isWindowSafe()) mainWindow.webContents.send("update-available", {
				version: info.version,
				releaseDate: info.releaseDate
			});
		});
		autoUpdater.on("update-downloaded", (info) => {
			if (isWindowSafe()) mainWindow.webContents.send("update-downloaded", { version: info.version });
		});
	...
	store.get("devExtensions", []).forEach(async (extPath) => {
		try {
			if (fs.default.existsSync(extPath)) await electron.session.defaultSession.loadExtension(extPath, { allowFileAccess: true });
		} catch (e) {
	...
	async function installCrxFromWebStore(extId, extName) {
		...
		try {
				...
				const doRequest = (url, depth = 0) => {
					...
					const req = (new URL(url).protocol === "https:" ? require("https") : require("http")).get(url, { headers: {
						...
						res.on("end", () => {
							...
							fs.default.writeFileSync(crxFile, buf);
							resolve();
						});
						res.on("error", reject);
					});
			...
			if (crxData.length < 1e3) {
				fs.default.unlink(crxFile, () => {});
				return {
					success: false,
					error: `Download incompleto (${crxData.length} bytes) — extensão pode não estar disponível`
				};
			}
			let zipStart = -1;
